...
export default function EditProduct() {
  ...
  const uploadFile = (e) => {
    ...
    .then(() => setUi(prev => ({...prev, disable: false})))
    .catch(err => console.log(err))
  }
  ...
  const submitProduct = (e) => {
    ...
    if(!data.title || !data.price || !data.description || !data.inventory || !data.image || !data.category ) {
      setUi({ text: 'Edit Product', color: 'purple', disable: false })
      return setErrMsg('Please provide all the necessary information')
    }

    let prodData = { title: data.title, price: Number(data.price), description: data.description, inventory: Number(data.inventory), image: data.image, category: data.category, freeShipping: data.freeShipping}
  ...
  return (
    <div className='editProduct'>
        <svg onClick={() => dispatch(uiActions.toggleEdit())} xmlns="http://www.w3.org/2000/svg" width="35" height="35" viewBox="0 0 24 24" ><path d="m16.192 6.344-4.243 4.242-4.242-4.242-1.414 1.414L10.535 12l-4.242 4.242 1.414 1.414 4.242-4.242 4.243 4.242 1.414-1.414L13.364 12l4.242-4.242z"></path></svg>
      <form onSubmit={submitProduct}>
        ...
        <input onChange={uploadFile} type='file' name='image' id='image' aria-label='image' />
